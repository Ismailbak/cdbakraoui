...
const suggestions = [
  ...
  },
  {
    ...
    description: 'Quels traitements recommanderais-tu pour une parodontite modérée ?',
    prompt: 'Quels traitements recommanderais-tu pour une parodontite modérée ?',
  },
  {
    icon: FiCalendar,
    title: 'Planifier un suivi',
    description: 'Quand devrais-je revoir un patient après un détartrage ?',
    prompt: 'Quand devrais-je revoir un patient après un détartrage ?',
  },
  {
    icon: FiTarget,
  ...
  const internalHandleSend = async (textToSend, fileToSend = null) => {
    ...
    try {
      ...
      const aiMsg = {
        ...
        model: res?.data?.model || res?.data?.metadata?.model || 'DentAI',
    ...
    setMessages(exchanges.flatMap((exchange) => {
      ...
      return [
        ...
        },
        {
          ...
          model: exchange.model_name || 'DentAI',
          timestamp: safeExchangeTimestamp,
        },
      ];
    }));
    ...
    <div className="assistant-shell">
      ...
      <section className="assistant-panel">
          ...
          <div className="assistant-message-list">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`assistant-message assistant-message-${msg.role}`}>
                ...
                <div className="assistant-bubble">
                  <div className="assistant-message-heading">
                    <strong>{msg.role === 'user' ? 'Vous' : msg.role === 'error' ? 'Erreur' : 'DentAI'}</strong>
        ...
        <footer className="assistant-composer-wrap">
          ...
          <p><FiZap /> DentAI peut faire des erreurs. Vérifiez les informations importantes.</p>
        </footer>
      </section>
