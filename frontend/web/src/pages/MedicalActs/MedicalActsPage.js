...
function DetailModal({ act, onClose }) {
  ...
  // Print handler - generates PDF and opens print dialog
  const handlePrint = () => {
    import('jspdf').then(jsPDF => {
      ...
      doc.text('Rapport Médical', 105, y, { align: 'center' });
      y += 12;

      // Patient info
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text(`Patient: ${act.patientName}`, 15, y);
      y += 8;
      doc.text(`Date: ${new Date(act.date).toLocaleDateString('fr-FR')}`, 15, y);
      y += 8;
      doc.text(`Type d'acte: ${act.type}`, 15, y);
      y += 8;
      doc.text(`Catégorie: ${act.category}`, 15, y);
      y += 12;
      ...
      if (act.diagnosis) {
        ...
        y += 6;
        doc.setFont('helvetica', 'normal');
        const diagLines = doc.splitTextToSize(act.diagnosis, 180);
      ...
      if (act.report) {
        ...
        y += 6;
        doc.setFont('helvetica', 'normal');
        const reportLines = doc.splitTextToSize(act.report, 180);
        doc.text(reportLines, 15, y);
        y += reportLines.length * 6 + 6;
      }

      // Treatment
      if (act.treatment) {
        doc.setFont('helvetica', 'bold');
        doc.text('Traitement:', 15, y);
        y += 6;
        doc.setFont('helvetica', 'normal');
        const treatLines = doc.splitTextToSize(act.treatment, 180);
        doc.text(treatLines, 15, y);
        y += treatLines.length * 6 + 6;
      }

      // Amount
      if (act.amount) {
        doc.setFont('helvetica', 'bold');
        doc.text(`Montant: ${act.amount} DH`, 15, y);
      }
